...
| **Amended by** | [DEC-011](DEC-011-cursor-oracle-access-cf-tunnel.md) (§5 access ask → **decided**; owner implementing — tunnel **not** claimed live) |
...
| **Access** | [DEC-011](DEC-011-cursor-oracle-access-cf-tunnel.md) (**decided**; owner implementing) |
...
1. **Host is live.** Oracle Always Free **`mal-core-0`** (Phoenix **AD-1**, **`VM.Standard.A1.Flex`**, **2 OCPU / 12 GB**, **aarch64**, Ubuntu **24.04** Minimal, 2 GB swap) is **provisioned and verified by Vaan**. Boot **50 GB**; data volume **`mal-core-data`** **150 GB** at **`/var/lib/mal`**. VCN **`mal-vcn`** `10.0.0.0/16`; subnet **`mal-public`** `10.0.1.0/24`; IGW **`mal-igw`**; NSG **`mal-core-nsg`**. SSH ingress = owner home public IP only; **no public app ports**. **Do not** claim agents already have SSH.
...
5. **No permanent PC networking dependency.** Desired path: Human → Grok → Cursor Cloud → `mal-core-0` → Postgres/JSONL/runtime. **Access decided in [DEC-011](DEC-011-cursor-oracle-access-cf-tunnel.md):** Cloudflare Tunnel (`cloudflared`) on `mal-core-0` + Cloudflare Access gating SSH + dedicated `mal-cursor` ed25519 deploy key (not the owner personal key). Backup: Tailscale **on the Oracle VM** + ephemeral agent auth keys. Owner **implements**. **Do not** claim the tunnel already exists. **Do not** expose Postgres publicly. **Do not** hand agents the owner’s personal SSH private key. **Do not** make the human PC a required always-on hop. **Do not** open SSH `:22` to the world. Cursor My Machines on-box is **parked** as phase-0 default.
...
- Access architecture is a **security-boundary** change: [DEC-011](DEC-011-cursor-oracle-access-cf-tunnel.md) locks CF Tunnel + Access + `mal-cursor` key; owner implements; no public DB, no owner-key shortcut, no PC-as-hop, no public `:22`.
...
- Granting agents SSH or deploying Tunnel/Access/Tailscale/keys (DEC-011 **decided**; **owner implementing** — not done)
...
- Owner implements DEC-011 (or falls back to Tailscale-on-Oracle / rejects). Until then, agents have **no** SSH.
