...
- **DEC-010/011:** `mal-core-0` **LIVE** (Phoenix AD-1, **2 OCPU / 12 GB A1**, Ubuntu 24.04, `/var/lib/mal` 150 GB). Postgres **16.15** `meme_core`/`mal_app` localhost. **No agent SSH yet.** Access **decided** ([DEC-011](../DEC/DEC-011-cursor-oracle-access-cf-tunnel.md)): CF Tunnel + Access + `mal-cursor` key — **pending owner implement** (do **not** claim tunnel exists). Handoff: [ORACLE-PHASE0-HANDOFF.md](ORACLE-PHASE0-HANDOFF.md).
...
- **Next owner:** **implement DEC-011** (hostname + `mal-cursor` as Cursor secret — never in repo); agents wait for confirm; then bootstrap dirs/schema/ingest. Human PC **must not** be a permanent hop.
