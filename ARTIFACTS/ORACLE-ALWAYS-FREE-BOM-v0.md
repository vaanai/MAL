...
1. **Owner ask:** implement [DEC-011](../DEC/DEC-011-cursor-oracle-access-cf-tunnel.md) (CF Tunnel + Access + `mal-cursor` key). **Not done.** **No** public Postgres (including via tunnel hostname); **no** owner personal SSH key to agents; **no** PC-as-permanent-hop; **no** `:22` to the world.
