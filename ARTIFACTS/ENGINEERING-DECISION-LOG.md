...
| **What was tested** | `python3 -m unittest tools.test_paper_laya_precompute_decision_packet_v0` (29 tests). Honest fixtures and `mixed-spines` example pass schema and CLI. Soft GATE FAIL #1 probe mutations fail schema and CLI. Prior honesty caps still fail both. Host-path refuses before `read_text` / `stat`. No RPC. `observe/client.py` untouched. |
