...
| **Verification** | Soft GATE **PASS** Formal-stamped Lyra (kill `bc-583eb833-6443-55f7-a685-b1ad3e166b6b`; head `cecdf6767c71d34da3927ba32646b674787eab45`; [Soft GATE comment](https://github.com/vaanai/MAL/pull/71#issuecomment-5813702684); Formal docs `ee88cd2` on `cursor/paper-laya-decision-packet-host-local-dry-run-receipt-capture-v0-d617`; FAIL #1 [closed](https://github.com/vaanai/MAL/pull/71#issuecomment-5813349728); FAIL #2 [closed](https://github.com/vaanai/MAL/pull/71#issuecomment-5813497572); [PR #71](https://github.com/vaanai/MAL/pull/71) squash `3d83622` on `main`). |
| **Current state** | **Proposed** registration **on `main`**. `measure.kind=none`; sealed book **incomplete**; Graph **cold**; `risk_gate.decision=locked`; `risk_gate.unlock=false`; `laya.authorize_run=false`; `laya.risk_gate_unlock=false`; `laya.live_trading=false`. |
...
| **What was tested** | `python3 -m unittest tools.test_paper_laya_decision_packet_batch_host_local_sealed_jsonl_dry_run_incomplete_rpc_v0` (**21 OK** per Soft GATE); parent #69 batch test **38 OK**; parent #68 **29 OK**; parent #67 **29 OK**. Honest fixtures pass CLI validator. Host-path refuses before `read_text` / parent invocation. FAIL #1–#2 **closed**. No RPC. `observe/client.py` untouched. |
| **Verification** | Soft GATE **PASS** Formal-stamped Lyra (kill `bc-6e0d511c-983b-59c3-8ca8-c388292ea5ed`; implement `bc-287de2e1-0979-5997-bc22-8e6288d630b6`; head `e0090890a1de5e3d8a5fd7d3aaab72a9cf3de1e5`; [Soft GATE comment](https://github.com/vaanai/MAL/pull/70#issuecomment-5813072359); FAIL #1 [closed](https://github.com/vaanai/MAL/pull/70#issuecomment-5812727576); FAIL #2 [closed](https://github.com/vaanai/MAL/pull/70#issuecomment-5812927365); Formal docs `e2ceac5` on `cursor/paper-laya-decision-packet-batch-host-local-sealed-jsonl-dry-run-incomplete-rpc-v0-30b6`; [PR #70](https://github.com/vaanai/MAL/pull/70) squash `d020fc9` on `main`). |
| **Current state** | **Proposed** registration **on `main`**. Caps: `measure.kind=none`; sealed book **incomplete**; Graph **cold**; `risk_gate.decision=locked`; `risk_gate.unlock=false`; `laya.authorize_run=false`; merge ≠ executed host dry-run. |
| **Rollback** | Revert PR; remove §11u from Lab memory. |
| **Unresolved** | — |
...
| **ID** | EDL-019 |
| **Date** | 2026-09-24 |
| **What changed** | Registered **Proposed** `paper-laya-decision-packet-batch-oracle-sealed-day-incomplete-rpc-v0`: [PAPER-LAYA-DECISION-PACKET-BATCH-ORACLE-SEALED-DAY-INCOMPLETE-RPC-V0.md](PAPER-LAYA-DECISION-PACKET-BATCH-ORACLE-SEALED-DAY-INCOMPLETE-RPC-V0.md), [paper-laya-decision-packet-batch-oracle-sealed-day-incomplete-rpc-v0.schema.json](paper-laya-decision-packet-batch-oracle-sealed-day-incomplete-rpc-v0.schema.json), fixtures under `fixtures/paper_laya_decision_packet_batch_oracle_sealed_day_incomplete_rpc_v0/`, `python -m tools.paper_laya_decision_packet_batch_oracle_sealed_day_incomplete_rpc_v0` (example / validate / batch), `tools/test_paper_laya_decision_packet_batch_oracle_sealed_day_incomplete_rpc_v0.py`. [LAB_STATE.md](../LAB_STATE.md) §11t. Parent #49–#68 CLIs not rewritten. |
| **Why** | Mirror #52 / #59 batch shape on validated #67 decision packets counted by #68 scoreboard path: synthetic day-aligned manifests → cite packets → per-day scoreboard + digest-stamp rollup without unlock, authorize-run, or live claims. |
| **What was tested** | `python3 -m unittest tools.test_paper_laya_decision_packet_batch_oracle_sealed_day_incomplete_rpc_v0` (38 tests per Soft GATE). Parent #68 test 29 OK; parent #67 test 29 OK; parent #63–#66 tests 15 / 19 / 22 / 30 OK per Soft GATE. Honest fixtures pass schema and CLI. Soft GATE FAIL #1–#5 holes **closed**; `schema_looser_than_cli` **CLOSED** for watched shapes. Host-path refuses before `read_text`. No RPC. `observe/client.py` untouched. |
| **Verification** | Soft GATE **PASS** Formal-stamped Lyra (kill `bc-5e4e8d4d-c120-59ce-915f-43ee1d7eedf1`; fix `bc-dd6817fa-6217-5dca-86b1-0449370f5d29`; head `e13fbd76a6c14c57af8d3cf1ca2ba517b0e3ee59`; [Soft GATE comment](https://github.com/vaanai/MAL/pull/69#issuecomment-5812462969); Formal docs `237522d` on `cursor/paper-laya-decision-packet-batch-oracle-sealed-day-incomplete-rpc-v0-c162`; [PR #69](https://github.com/vaanai/MAL/pull/69) squash `adadaa8` on `main`). |
| **Current state** | **Proposed** registration **on `main`**. Caps: `measure.kind=none`; sealed book **incomplete**; Graph **cold**; `risk_gate.decision=locked`; `risk_gate.unlock=false`; `laya.authorize_run=false`; `laya.live_trading=false`. |
| **Rollback** | Revert PR; remove §11t from Lab memory. |
| **Unresolved** | — |
| **Implications** | Next LAYA citeable batch step after #68 scoreboard; does not unlock risk gate or authorize LAYA run. |
| **Pointers** | [#67 decision packet](PAPER-LAYA-PRECOMPUTE-DECISION-PACKET-V0.md), [#68 scoreboard](PAPER-LAYA-DECISION-PACKET-SCOREBOARD-SEALED-FIXTURE-V0.md), [#52 batch](PAPER-BATCH-ORACLE-SEALED-DAY-INCOMPLETE-RPC-V0.md), [#59 fill-sim batch](PAPER-FILL-SIM-BATCH-ORACLE-SEALED-DAY-INCOMPLETE-RPC-V0.md). |
...
| **What was tested** | `python3 -m unittest tools.test_paper_laya_decision_packet_scoreboard_sealed_fixture_v0` (29 tests per Soft GATE). Parent #67 test 29 OK; parent #63–#66 tests 15 / 19 / 22 / 30 OK per Soft GATE. Honest fixtures pass schema (with decision-packet schema registry) and CLI. Soft GATE FAIL #1 holes **closed**; `schema_looser_than_cli` **CLOSED** for watched shapes. Dishonest unlock/authorize/live, closed book, measure kind, numeric horizons, and aggregate drift fail schema and CLI. Host-path refuses before `read_text`. No RPC. `observe/client.py` untouched. |
| **Verification** | Soft GATE **PASS** Formal-stamped Lyra (kill `bc-2fd452de-34a6-5c47-8076-3840fb2b6af7`; fix `bc-1908e01a-bed7-5580-b561-10eb7e67b9cd`; head `e52fb490589cc0246589edd2444f7d3f866c0c64`; [Soft GATE comment](https://github.com/vaanai/MAL/pull/68#issuecomment-5811344383); Formal docs `cbf5bf7` on `cursor/paper-laya-decision-packet-scoreboard-sealed-fixture-v0-5a3e`; [PR #68](https://github.com/vaanai/MAL/pull/68) squash `d31af86` on `main`). |
| **Current state** | **Proposed** registration **on `main`**. Caps: `measure.kind=none`; sealed book **incomplete**; Graph **cold**; `risk_gate.decision=locked`; `risk_gate.unlock=false`; `laya.authorize_run=false`; `laya.live_trading=false`. |
...
| **What was tested** | `python3 -m unittest tools.test_paper_laya_precompute_decision_packet_v0` (29 tests). Parent #63–#65 unit tests 15 / 19 / 22 OK (85 combined per Soft GATE). Honest fixtures and `mixed-spines` example pass schema and CLI. Soft GATE FAIL #1 probe mutations fail schema and CLI. Prior honesty caps still fail both. Host-path refuses before `read_text` / `stat`. No RPC. `observe/client.py` untouched. |
| **Verification** | Soft GATE **PASS** Formal-stamped Lyra (kill `bc-84b8db98-a158-5530-8e30-9e33587231fa`; fix `bc-0361a565-20bd-5763-bd67-a6a9a1034c57`; implement `bc-d0648945-6ec4-5891-91e6-0d94f7bd4056`; head `f2e4508678bff71a1c55e990f2ce52dae2aa2b0a`; [Soft GATE comment](https://github.com/vaanai/MAL/pull/67#issuecomment-5810922748); Formal docs `e935222` on `cursor/paper-laya-precompute-decision-packet-v0-4056`; [PR #67](https://github.com/vaanai/MAL/pull/67) squash `52a06e7` on `main`; `python3 -m unittest tools.test_paper_laya_precompute_decision_packet_v0` — 29 OK; parent #63–#65 tests 15 / 19 / 22 OK per Soft GATE). |
| **Current state** | **Proposed** registration **on `main`**. Not a measure. Not LAYA authorize-run. Graph cold. Sealed book incomplete. `risk_gate.unlock=false`. |
...
| **ID** | EDL-017 |
| **Date** | 2026-09-24 |
| **What changed** | Registered **Proposed** `paper-laya-decision-packet-incomplete-rpc-honesty-schema-align-v0`: [PAPER-LAYA-DECISION-PACKET-INCOMPLETE-RPC-HONESTY-SCHEMA-ALIGN-V0.md](PAPER-LAYA-DECISION-PACKET-INCOMPLETE-RPC-HONESTY-SCHEMA-ALIGN-V0.md). Tightened #70–#71 `soft_watches.items` from enum bags to ordered `prefixItems` with full cardinality (47 / 50). Soft GATE FAIL #1 on [PR #72](https://github.com/vaanai/MAL/pull/72): #70 non-operator invocation paths → checked-in `prefixItems` `const`s; `executed_by_this_process` locked per `receipt_kind`. #67–#69 citeable schemas already matched CLI on watched shapes from Soft GATE; tests cite them. No new runtime schema. No CLI rewrite. |
...
| **Verification** | Soft GATE **PASS** Formal-stamped Composer (kill `bc-bc2ab492-9f94-57be-bcd2-90e93bb105b3`; head `28ee8e3deb7779e67e564566ad3509594909ad45`; [Soft GATE comment](https://github.com/vaanai/MAL/pull/72#issuecomment-5814534435); FAIL #1 [closed](https://github.com/vaanai/MAL/pull/72#issuecomment-5814090373); FAIL #2 [closed](https://github.com/vaanai/MAL/pull/72#issuecomment-5814297367); Formal docs `018c79a` on `cursor/paper-laya-decision-packet-incomplete-rpc-honesty-schema-align-57e4`; [PR #72](https://github.com/vaanai/MAL/pull/72) squash on `main` pending); `python3 -m unittest tools.test_paper_laya_decision_packet_incomplete_rpc_honesty_schema_align_v0` — **16 OK**; parent #67–#71 tests **29 / 29 / 38 / 21 / 17 OK** per Soft GATE. |
| **Current state** | **Proposed** registration **on `main`** after Formal merge. Not a measure. Not risk-gate unlock. Not LAYA authorize-run. Graph cold. Sealed book incomplete. `risk_gate.unlock=false`. `schema_looser_than_cli` **CLOSED** for watched decision-packet shapes. |
...
| **ID** | EDL-016 |
| **Date** | 2026-09-24 |
| **What changed** | Registered **Proposed** `paper-laya-incomplete-rpc-honesty-schema-align-v0`: [PAPER-LAYA-INCOMPLETE-RPC-HONESTY-SCHEMA-ALIGN-V0.md](PAPER-LAYA-INCOMPLETE-RPC-HONESTY-SCHEMA-ALIGN-V0.md). Tightened #63–#65 schemas: `soft_watches.items` enums and full-list cardinality; anchored digest `repo_path` / graph-status tie; `repoFixturePathHonesty` on assembly and lock-receipt paths; DEC-007 `fill_sim_status_counts` paired with `fixture_path` + `registration` spine on lock receipt. Soft GATE FAIL #1–#4 holes **closed** on [PR #66](https://github.com/vaanai/MAL/pull/66); `repoFixturePathHonesty` refuses lexical path-collapse segments (`//`, trailing `/`, `.` segments, backslashes) inside allowed fixture prefixes. No new runtime schema. No CLI rewrite. |
...
| **Verification** | Soft GATE **PASS** Formal-stamped Lyra (kill `bc-b59bd3fd-d786-5475-83f6-02f421b95098`; fix `bc-ef9eef8d-8dc0-5340-a1d3-563926df1b6e`; head `81be82c7c6dd2ea69a6c6163f1215fd23271b0a1`; [Soft GATE comment](https://github.com/vaanai/MAL/pull/66#issuecomment-5810546271); Formal docs `2a4ed13` on `cursor/paper-laya-incomplete-rpc-honesty-schema-align-d929`; [PR #66](https://github.com/vaanai/MAL/pull/66) squash `3128128` on `main`; `python3 -m unittest tools.test_paper_laya_incomplete_rpc_honesty_schema_align_v0` — 30 OK; parent #63–#65 tests 15 / 19 / 22 OK per Soft GATE). |
| **Current state** | **Proposed** registration **on `main`**. Not a measure. Not risk-gate unlock. Not LAYA authorize-run. Graph cold. Sealed book incomplete. `risk_gate.unlock=false`. |
