...
| **ID** | EDL-016 |
| **Date** | 2026-09-24 |
| **What changed** | Registered **Proposed** `paper-laya-incomplete-rpc-honesty-schema-align-v0`: [PAPER-LAYA-INCOMPLETE-RPC-HONESTY-SCHEMA-ALIGN-V0.md](PAPER-LAYA-INCOMPLETE-RPC-HONESTY-SCHEMA-ALIGN-V0.md). Tightened #63–#65 schemas: `soft_watches.items` enums and full-list cardinality; anchored digest `repo_path` / graph-status tie; `repoFixturePathHonesty` on assembly and lock-receipt paths; DEC-007 `fill_sim_status_counts` paired with `fixture_path` + `registration` spine on lock receipt. Soft GATE FAIL #1–#4 holes **closed** on draft PR #66 (Formal Soft GATE still **pending**); `repoFixturePathHonesty` refuses lexical path-collapse segments (`//`, trailing `/`, `.` segments, backslashes) inside allowed fixture prefixes. No new runtime schema. No CLI rewrite. |
