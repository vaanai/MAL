...
| [paper-laya-decision-packet-batch-host-local-sealed-jsonl-dry-run-incomplete-rpc-v0.schema.json](paper-laya-decision-packet-batch-host-local-sealed-jsonl-dry-run-incomplete-rpc-v0.schema.json) | **`soft_watches.items`:** enum bag → ordered `prefixItems` (47) + `uniqueItems` + `items: false`. Other honesty bits were already `const` on #70. |
