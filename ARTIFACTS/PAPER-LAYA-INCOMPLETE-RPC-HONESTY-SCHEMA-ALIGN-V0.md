...
| Graph revive claims: `graph_lane_revived=true`, `graph_policy` warm, scored `graph_lift`, dishonest `graph_lift_aggregate_status` | **Soft GATE FAIL #2 hole closed:** `graph_lift_aggregate_status` on scoreboard digests is tied to `citation.repo_path` via `if` / `then` (mirrors #61 cold-bit tie on fill-sim stamps). In-enum swaps such as `not_used_graph_cold` ↔ `not_scored_mixed` fail schema. Out-of-enum `graph_scored_lift` still fails both. Other Graph caps were already `const`. |
| Lock-receipt host paths (`fixture_path`, `input.assembly.surround_paths`) under `/var/lib/mal` or lexical `var/lib/mal` / `..` forms | **Soft GATE FAIL #2 hole closed:** `layaSurroundFixturePath` pattern-locks to `fixtures/paper_laya_precompute_{surround,fill_sim_surround}_packet_v0/` and `not` / `pattern` refuses absolute, doubled-slash, relative, and `../lib/mal` host-root shapes (#54/#61 style). |
| Non-fill lock-receipt citation carrying `fill_sim_status_counts` | **Soft GATE FAIL #2 hole closed:** `if` / `then` on `registration.id == paper-laya-precompute-surround-packet-v0` forbids `digest.fill_sim_status_counts` (`not: { required: [...] }`). Fill-sim citations still require the key. |
...
| Schema | Diff |
| --- | --- |
| [paper-laya-precompute-surround-packet-v0.schema.json](paper-laya-precompute-surround-packet-v0.schema.json) | `soft_watches.items` enum plus `minItems` / `maxItems` 17; `scoreboardDigest` `allOf` ties `graph_lift_aggregate_status` to cited scoreboard `repo_path`. Other honesty bits were already `const` on #64. |
| [paper-laya-precompute-fill-sim-surround-packet-v0.schema.json](paper-laya-precompute-fill-sim-surround-packet-v0.schema.json) | `soft_watches.items` enum plus `minItems` / `maxItems` 29; same digest graph-status tie. Top-level `fill_sim_status_counts` DEC-007 `prefixItems` were already on #63. |
| [paper-laya-risk-gate-lock-receipt-v0.schema.json](paper-laya-risk-gate-lock-receipt-v0.schema.json) | `layaSurroundFixturePath` on `fixture_path` and `surround_paths`; `fill_sim_status_counts` DEC-007 `prefixItems`; `if` / `then` requires the key on fill-sim citations and forbids it on non-fill citations; `soft_watches.items` enum plus `minItems` / `maxItems` 34. Lock caps were already on #65. |
