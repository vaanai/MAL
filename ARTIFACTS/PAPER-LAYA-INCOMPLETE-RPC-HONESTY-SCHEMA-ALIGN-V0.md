...
| Assembly / lock-receipt paths with empty segments (`//`, `///`), `.` segments (`/./`), trailing `/`, or backslashes inside the allowed `fixtures/...` prefix | **Soft GATE FAIL #4 hole closed:** `repoFixturePathHonesty` also refuses `//`, `/\$`, `(^|/)\.(/|$)`, and `\\` so schema matches the CLI exact-path check after `Path.resolve()` (`rel != cited string` → `missing or refused path`). |
