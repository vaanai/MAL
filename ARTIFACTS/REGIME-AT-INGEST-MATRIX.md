...
| **Fee regime** | No | Yes (Global + per-coin) | Default uncertain at create | `fee=unverified`; `fee=global_100bps` if RPC confirms **100** bps; **`fee=global_95bps` Proposed (EXP-007d)** if RPC confirms **95** bps at create slot — never map 95→100 | Changing fee tag after trades executed under old label |
