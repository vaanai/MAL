...
| Whole-number float on an integer slot (`1.0`, `2.0` on `prior_mint_count`), including when the packet is nested by `$ref` under paper-evaluate and paper-scoreboard | `jsonIntegerOrNull` is a draft-04 schema. That is the dialect the slot `$ref` selects, including from a stock draft 2020-12 parent. Draft-04 integer rejects `1.0` and `2.0`. A JSON integer such as `2` still passes. |
