...
Runtime Secrets (must already be in the environment; never pass on CLI):
...
Optional:
  ...
  MAL_SSH_LOCAL_LISTEN      default 127.0.0.1:2222
...
if ! python3 -c "import socket,sys; s=socket.socket(); s.settimeout(1); s.connect(('127.0.0.1', int(sys.argv[1]))); s.close()" "${HOSTPORT}" 2>/dev/null; then
  "${CF_BIN}" access tcp --hostname "${ACCESS_HOST}" --url "${LOCAL_LISTEN}" \
    --id "${CLOUDFLARE_ACCESS_CLIENT_ID}" --secret "${CLOUDFLARE_ACCESS_CLIENT_SECRET}" \
...
ssh -i "${KEYFILE}" -p "${HOSTPORT}" \
  ...
  -o UserKnownHostsFile="${MAL_SSH_KNOWN_HOSTS:-/tmp/mal-oracle-known_hosts}" \
