...
| [mal-funding-graph.service](mal-funding-graph.service) | host (systemd --user) | Nice'd enricher. 1/s on public RPC. Helius when `HELIUS_API_KEY` or the backfill env file appears (`MAL_FUNDING_RPS` overrides the Helius rate, keep it ≤ 10) unless `MAL_FUNDING_RPC=public`. Stops Helius at 2M credits and falls back to public 1/s |
