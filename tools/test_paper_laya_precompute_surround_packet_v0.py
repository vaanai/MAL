"""Schema and CLI parity for paper-laya-precompute-surround-packet-v0."""
...
FIXTURES = ROOT / "fixtures" / "paper_laya_precompute_surround_packet_v0"
SCHEMA = ROOT / "ARTIFACTS" / "paper-laya-precompute-surround-packet-v0.schema.json"
MODULE = ROOT / "tools" / "paper_laya_precompute_surround_packet_v0.py"
...
class PaperLayaPrecomputeSurroundPacketV0Tests(unittest.TestCase):
    ...
    def test_schema_locks_honesty_caps(self) -> None:
        ...
        self.assertNotIn("fill_sim_status_counts", schema["properties"])
        prefix = schema["$defs"]["labelRates"]["properties"]["rows"]["prefixItems"]
        self.assertEqual(prefix[0]["properties"]["label"]["const"], "runner")
        self.assertEqual(prefix[1]["properties"]["label"]["const"], "reject")
        self.assertEqual(
    ...
    def test_dishonest_laya_live_trading_fails_schema_and_cli(self) -> None:
        doc = copy.deepcopy(example_mixed_scoreboard())
    ...
    def test_host_triple_slash_path_refused_without_stat(self) -> None:
        target = f"///var/lib/mal/paper/surround.json"
        with mock.patch("pathlib.Path.read_text") as read_text:
            code = _quiet(["validate", target])
        self.assertEqual(code, 1)
    ...
    def test_dot_slash_var_lib_mal_refused_without_stat(self) -> None:
        with mock.patch("os.getcwd", return_value="/"):
            with mock.patch("pathlib.Path.read_text") as read_text:
                code = _quiet(["validate", "./var/lib/mal/paper/surround.json"])
        self.assertEqual(code, 1)
    ...
    def test_parent_dotdot_var_lib_mal_refused_without_stat(self) -> None:
        with mock.patch("os.getcwd", return_value="/var/lib"):
            with mock.patch("pathlib.Path.read_text") as read_text:
                code = _quiet(["validate", "../lib/mal/paper/surround.json"])
        self.assertEqual(code, 1)
        read_text.assert_not_called()
