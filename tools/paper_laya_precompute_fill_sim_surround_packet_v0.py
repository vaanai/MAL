"""paper-laya-precompute-fill-sim-surround-packet-v0.

Proposed registration. Assembles validated paper_fill_sim_scoreboard_sealed_fixture_v0
digests (and optionally cites #59 batch rollup fields) into a fixtures-only surround
packet shaped for later LAYA precompute consumption.
...
SCHEMA_VERSION = "paper_laya_precompute_fill_sim_surround_packet_v0"
PACKET_TYPE = "paper_laya_precompute_surround_packet"
PACKET_ID = "paper-laya-precompute-fill-sim-surround-packet-v0"
...
SCOREBOARD_PARENT_COMMIT = "dfbab7a"
SCOREBOARD_PARENT_PR = 58
BATCH_PARENT_COMMIT = "4664363"
BATCH_PARENT_PR = 59
...
SOFT_WATCH_SOURCE = (
    ...
    "laya_precompute_fill_sim_surround_packet_v0"
)
...
ALLOWED_SCOREBOARD_PREFIX = "fixtures/paper_fill_sim_scoreboard_sealed_fixture_v0/"
ALLOWED_BATCH_PREFIX = "fixtures/paper_fill_sim_batch_oracle_sealed_day_incomplete_rpc_v0/"
FIXTURE_DIR = REPO / "fixtures" / "paper_laya_precompute_fill_sim_surround_packet_v0"
...
def _scoreboard_digest(board: Mapping[str, Any], repo_path: str) -> dict[str, Any]:
    ...
    return {
        ...
        "fill_sim_status_counts": [
            ...
            for row in board["fill_sim_status_counts"]
        ],
...
def _aggregate_fill_sim(rows_list: Sequence[Sequence[Mapping[str, Any]]]) -> list[dict[str, Any]]:
    totals: dict[str, int] = {FILL_SIM_RUNNER: 0, FILL_SIM_REJECT: 0}
    for rows in rows_list:
        for row in rows:
            status = row.get("status")
            if status in totals:
                totals[str(status)] += int(row.get("n", 0))
    total_n = totals[FILL_SIM_RUNNER] + totals[FILL_SIM_REJECT]
    return [
        {
            "status": FILL_SIM_RUNNER,
            "n": totals[FILL_SIM_RUNNER],
            "share": _share(totals[FILL_SIM_RUNNER], total_n or 1),
        },
        {
            "status": FILL_SIM_REJECT,
            "n": totals[FILL_SIM_REJECT],
            "share": _share(totals[FILL_SIM_REJECT], total_n or 1),
        },
    ]
...
def assemble(
    *,
    ...
    # De-duplicate boards by repo_path keeping first
    seen: set[str] = set()
    ...
    fill_rows = _aggregate_fill_sim([d["fill_sim_status_counts"] for d in digests])
    ...
    packet = {
        ...
        "fill_sim_status_counts": fill_rows,
        "horizons": {
            "status": HORIZON_STATUS,
            "values_supplied": False,
            "null_is_not_zero_return": True,
...
def example_mixed_scoreboard() -> dict[str, Any]:
    path = "fixtures/paper_fill_sim_scoreboard_sealed_fixture_v0/mixed_runner_reject.json"
...
def example_batch_two_day_with_digest() -> dict[str, Any]:
    batch_path = "fixtures/paper_fill_sim_batch_oracle_sealed_day_incomplete_rpc_v0/two_day.json"
...
def main(argv: list[str] | None = None) -> int:
    parser = _Parser(
        prog="python -m tools.paper_laya_precompute_fill_sim_surround_packet_v0",
        description=(
            "Assemble validated fill-sim scoreboard digests into a LAYA precompute "
