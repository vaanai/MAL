...
class ResidualArmMathTests(unittest.TestCase):
    def test_arm_row_in_cohort_minus_v2(self) -> None:
        ...
        class _Row:
            def __init__(self, sig: str, feats: GraphFeatures) -> None:
                self.signature = sig
                self.features = feats

        row = _Row("SigA", base)
        v2 = {"SigB"}
        self.assertTrue(arm_row_in_cohort(row, "L3_minus_v2", v2))
        self.assertFalse(arm_row_in_cohort(row, "L3_intersect_v2", v2))
        row_v2 = _Row("SigB", base)
        self.assertFalse(arm_row_in_cohort(row_v2, "L3_minus_v2", v2))
        self.assertTrue(arm_row_in_cohort(row_v2, "L3_intersect_v2", v2))
...
class Exp005IntegrationTests(unittest.TestCase):
    ...
    def test_run_exp005b_residual_arms_and_gates(self) -> None:
        r0 = _create_row(t_ws=T0, mint="M1", signature="Sig1")
        r1 = _create_row(t_ws=T1, mint="M2", signature="Sig2", trader="CreatorPk1")
        r2 = _create_row(t_ws=T2, mint="M3", signature="Sig3", trader="CreatorPk1")
        marks = [
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            summary = run_exp005b(
                ...
                marks_paths=[mpath],
                output_dir=out_dir,
                prefix="_exp005b_test",
            )
            self.assertEqual(summary["exp"], "EXP-005b-l3-residual-vs-exp002c-v2-falsifier-v0")
            arms = summary["residual_arms"]
            self.assertIn("L3_minus_v2", arms)
            self.assertIn("L3_intersect_v2", arms)
            self.assertIn("L3_full", arms)
            self.assertIn("exp002c_runner_overlap", summary)
            report = (out_dir / "_exp005b_test_report.md").read_text()
            self.assertIn("L3_minus_v2", report)
