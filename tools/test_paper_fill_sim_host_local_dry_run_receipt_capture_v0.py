"""Schema and CLI parity for paper-fill-sim-host-local-dry-run-receipt-capture-v0.
...
from tools.paper_fill_sim_host_local_dry_run_receipt_capture_v0 import (
    SOFT_WATCH_ITEMS,
    main,
    validate_capture,
)
...
FIXTURES = ROOT / "fixtures" / "paper_fill_sim_host_local_dry_run_receipt_capture_v0"
SCHEMA = (
    ROOT / "ARTIFACTS" / "paper-fill-sim-host-local-dry-run-receipt-capture-v0.schema.json"
)
MODULE = ROOT / "tools" / "paper_fill_sim_host_local_dry_run_receipt_capture_v0.py"
PARENT_MODULE = (
    ROOT / "tools" / "paper_fill_sim_batch_host_local_sealed_jsonl_dry_run_incomplete_rpc_v0.py"
)
...
class PaperFillSimHostLocalDryRunReceiptCaptureV0Tests(unittest.TestCase):
    ...
    def test_schema_locks_honesty_caps(self) -> None:
        ...
        arms = schema["$defs"]["fullBook"]["properties"]["fill_sim_status_arms"]["prefixItems"]
        self.assertEqual(arms[0]["const"], FILL_SIM_RUNNER)
        self.assertEqual(arms[1]["const"], FILL_SIM_REJECT)
        self.assertEqual(schema["$defs"]["parentDryRun"]["properties"]["commit"]["const"], "d3b5554")
        self.assertEqual(schema["$defs"]["parentDryRun"]["properties"]["pull_request"]["const"], 60)
        self.assertEqual(schema["$defs"]["honestyAlign"]["properties"]["commit"]["const"], "677e88b")
    ...
    def test_receipt_citation_does_not_embed_a_body(self) -> None:
        ...
        self.assertIsNone(declared["recorded"]["fill_sim_status_arms_on_receipt"])
        replay = _load(FIXTURES / "receipt_synthetic_replay.json")
        self.assertEqual(
            replay["recorded"]["fill_sim_status_arms_on_receipt"],
    ...
    def test_module_does_not_import_observe(self) -> None:
        ...
        self.assertNotIn(
            "paper_fill_sim_host_local_dry_run_receipt_capture",
        ...
        self.assertNotIn("paper_fill_sim_host_local_dry_run_receipt_capture", parent)
