"""paper-host-local-dry-run-receipt-capture-v0.

Proposed registration. Citeable capture of refuse and receipt outcomes from
paper_batch_host_local_sealed_jsonl_dry_run_incomplete_rpc_v0.
...
SCHEMA_VERSION = "paper_host_local_dry_run_receipt_capture_v0"
CAPTURE_TYPE = "paper_dry_run_capture"
CAPTURE_ID = "paper-host-local-dry-run-receipt-capture-v0"
PARENT_COMMIT = "f17b59f"
PARENT_PR = 53
PARENT_CLI = (
    "python -m tools.paper_batch_host_local_sealed_jsonl_dry_run_incomplete_rpc_v0"
)
ALIGN_ID = "paper-incomplete-rpc-honesty-schema-align-v0"
ALIGN_COMMIT = "ca4e815"
ALIGN_PR = 54
...
SOFT_WATCH_SOURCE = (
    ...
    "batch_pr52_dry_run_pr53_schema_align_pr54"
)
INHERITED_FROM: tuple[str, ...] = (
    ...
    "paper_batch_host_local_sealed_jsonl_dry_run_incomplete_rpc_v0_pr53",
    "paper_incomplete_rpc_honesty_schema_align_v0_pr54",
)
...
RECEIPT_PATHS: dict[str, str] = {
    "synthetic_replay": (
        "fixtures/paper_batch_host_local_sealed_jsonl_dry_run_incomplete_rpc_v0/"
...
OUTSIDE_REPO = "/tmp/mal-paper-capture-outside-repo"
HOST_VALIDATE_PATH = (
    f"{HOST_ROOT}/paper/{RECEIPT_ID}/receipt.json"
)
...
def _host_path_forms() -> list[dict[str, Any]]:
    ...
    return [
        ...
        },
        {
            ...
            "cwd": None,
            "jsonl": [
                f"{OUTSIDE_REPO}/observe-{day}.jsonl" for day in COURIER_DAYS
            ],
            "expectation": [
                f"{OUTSIDE_REPO}/sealed_day_{day}_expectation.json"
...
def _run_dry(argv: list[str], cwd: str | None) -> tuple[int, str, list[str], list[object]]:
    """Call the parent dry-run CLI. Count read_text and parent_main calls."""
    reads: list[str] = []
...
def capture_receipt(receipt_kind: str) -> tuple[dict[str, Any] | None, list[str]]:
    """Cite one checked-in #53 receipt. The body is not copied onto this object."""
...
def capture_refuse_host_path() -> tuple[dict[str, Any] | None, list[str]]:
    """Re-execute the #53 host-open refuses. No path contents are stored."""
...
def _refuse_capture_path(text: str) -> str | None:
    """Same open-refuse as the parent dry-run validate, before read_text."""
...
def main(argv: list[str] | None = None) -> int:
    parser = _Parser(
        prog="python -m tools.paper_host_local_dry_run_receipt_capture_v0",
        description=(
            "Capture citeable refuse and receipt outcomes from the host-local "
            "dry-run. Sealed book stays incomplete. No /var/lib/mal read. "
            "No measure exit."
        ),
    )
