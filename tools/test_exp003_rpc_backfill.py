...
def _tx(sig: str, block_time: int, market_cap: float) -> dict[str, Any]:
    return {
...
class PriceExtractTests(unittest.TestCase):
    def test_market_cap_preferred(self) -> None:
        tx = _tx("s", T_UNIX + 2, 42.0)
        tx["vSolInBondingCurve"] = 10.0
        price, field = price_from_transaction(tx)
...
class MockRpcTests(unittest.TestCase):
    ...
    def test_process_create_writes_valid_marks(self) -> None:
        ...
        txs = {
            "sigA": _tx("sigA", T_UNIX + 1, 101.0),
            "sigB": _tx("sigB", T_UNIX + 10, 110.0),
            "sigC": _tx("sigC", T_UNIX + 70, 200.0),
        }
        ...
        ticks = [
            (datetime.fromtimestamp(T_UNIX + 1, tz=timezone.utc), 101.0),
            (datetime.fromtimestamp(T_UNIX + 10, tz=timezone.utc), 110.0),
        ]
        chosen_1s = select_last_as_of(ticks, t_decision, 1.0)
        self.assertIsNotNone(chosen_1s)
        assert chosen_1s is not None
        self.assertEqual(chosen_1s[1], 101.0)
    ...
    def test_run_backfill_appends_jsonl(self) -> None:
        ...
        txs = {"sigA": _tx("sigA", T_UNIX + 3, 105.0)}
