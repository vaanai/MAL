"""paper-fill-sim-batch-host-local-sealed-jsonl-dry-run-incomplete-rpc-v0.

Proposed paper registration. A thin receipt for the operator-local dry-run
shape of paper_fill_sim_batch_oracle_sealed_day_incomplete_rpc_v0 against
day-aligned sealed observe JSONL on mal-core-vnic.
...
from tools.paper_batch_oracle_sealed_day_incomplete_rpc_v0 import EXAMPLE_DAYS
...
SCHEMA_VERSION = "paper_fill_sim_batch_host_local_sealed_jsonl_dry_run_incomplete_rpc_v0"
RECEIPT_TYPE = "paper_dry_run_receipt"
RECEIPT_ID = "paper-fill-sim-batch-host-local-sealed-jsonl-dry-run-incomplete-rpc-v0"
PARENT_COMMIT = "4664363"
PARENT_PR = 59
PARENT_CLI = "python -m tools.paper_fill_sim_batch_oracle_sealed_day_incomplete_rpc_v0"
HOST = "mal-core-vnic"
HOST_ROOT = "/var/lib/mal"
INCOMPLETE_REASON = "fill_sim_host_local_dry_run_does_not_close_the_sealed_book"
...
SOFT_WATCH_SOURCE = (
    ...
    "fill_sim_batch_host_local_sealed_jsonl_dry_run_incomplete_rpc_v0"
)
...
LOCAL_SOFT_WATCH_ITEMS: tuple[str, ...] = (
    ...
    "fill_sim_host_local_not_a_host_extract",
...
SYNTHETIC_JSONL: dict[str, str] = {
    day: (
        "fixtures/paper_fill_sim_batch_oracle_sealed_day_incomplete_rpc_v0/"
        f"observe-{day}.jsonl"
    )
    for day in COURIER_DAYS
}
...
def _argv_shape(origin: str, jsonl: Sequence[str], expectation: Sequence[str]) -> list[str]:
    argv = [
        "python",
        "-m",
        "tools.paper_fill_sim_batch_oracle_sealed_day_incomplete_rpc_v0",
        "batch",
        "--fixture-origin",
        origin,
    ]
...
def _fill_sim_status_arms(batch: Mapping[str, Any]) -> list[str] | None:
    days = batch.get("days")
    ...
    if not isinstance(first, Mapping):
        return None
    board = first.get("scoreboard")
    if not isinstance(board, Mapping):
        return None
    rows = board.get("fill_sim_status_counts")
    ...
    for row in rows[:2]:
        ...
        if not isinstance(status, str):
            return None
        arms.append(status)
    if arms != [FILL_SIM_RUNNER, FILL_SIM_REJECT]:
        return None
    for day_block in days:
        if not isinstance(day_block, Mapping):
            return None
        scoreboard = day_block.get("scoreboard")
        if not isinstance(scoreboard, Mapping):
            return None
        day_rows = scoreboard.get("fill_sim_status_counts")
        if not isinstance(day_rows, list):
            return None
        day_arms = [
            row.get("status")
            for row in day_rows[:2]
...
def _parent_is_honest(batch: Mapping[str, Any]) -> bool:
    ...
    input_ok = (
        isinstance(input_block, Mapping) and input_block.get("marks_joined") is False
    )
    arms_ok = _fill_sim_status_arms(batch) is not None
    return identity_ok and graph_ok and dual_ok and measure_ok and honesty_ok and input_ok and arms_ok


def _digest(batch: Mapping[str, Any]) -> dict[str, Any]:
    arms = _fill_sim_status_arms(batch)
...
def assemble_receipt(
    *,
    ...
    for day in COURIER_DAYS:
        ...
        if _batch_path_refusal(jsonl_text) or _batch_path_refusal(expectation_text):
            return None, [
    ...
    if not _parent_is_honest(batch):
        return None, ["parent batch is not an incomplete-RPC fill-sim paper batch"]
...
def main(argv: list[str] | None = None) -> int:
    parser = _Parser(
        prog="python -m tools.paper_fill_sim_batch_host_local_sealed_jsonl_dry_run_incomplete_rpc_v0",
        description=(
            "Receipt for an operator-local dry-run of the fill-sim sealed-day paper batch. "
    ...
    receipt_cmd = sub.add_parser(
        "receipt",
        help="Wrap the parent fill-sim batch CLI on in-repo files and print a receipt",
    )
