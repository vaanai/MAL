...
- **Platform-regime taxonomy (bonding-first v0):** **EXP-007** sealed **coverage / knowable-at-T** audit **2026-09-23** on courier **2026-09-20/21** → cross-day **INCOMPLETE** (structural gates **PASS**; **K-platform-rpc-resolved INCOMPLETE** — 100% WS defaults). **EXP-007b** Helm **2026-09-23** — capped (≤500/day) RPC `regime_enrich` + audit re-score ([`tools.exp007_rpc_enrich`](tools/exp007_rpc_enrich.py), [`tools.exp007_regime_audit`](tools/exp007_regime_audit.py) `--enrich-jsonl`); host `_exp007b-oracle-*`. **Wiring / encoder still Proposed** — merge ≠ continuous observe-wiring. [EXP-007](EXP/EXP-007-platform-regime-taxonomy-v0.md) · [EXP-007b](EXP/EXP-007b-platform-regime-rpc-enrich-v0.md); enum [REGIME-ENUM-V0.md](ARTIFACTS/REGIME-ENUM-V0.md).
...
9b. **EXP-007b RPC enrich + audit re-score (scored 2026-09-23):** Oracle `mal-core-vnic` — **500**/day enrich written both courier days; **leak_reject_n=0**; cross-day enriched overall **INCOMPLETE** (**K-platform-rpc-resolved** still **INCOMPLETE** — fee 100% `unverified`; instr pending **12–16%**; quote_verified **70–76%** on sample). Sealed full-book stays **INCOMPLETE**. CLI [`tools.exp007_rpc_enrich`](tools/exp007_rpc_enrich.py); runbook [`tools/exp007b_oracle_run.md`](tools/exp007b_oracle_run.md). Host `/var/lib/mal/paper/_exp007b-oracle-*`. **Merge ≠ continuous observe-wiring.** [EXP-007b](EXP/EXP-007b-platform-regime-rpc-enrich-v0.md).
