...
9b. **EXP-007b RPC enrich + audit re-score (Helm 2026-09-23):** Capped ≤**500** creates/day knowable-at-T RPC enrich → append-only `regime_enrich`; re-run audit with `--enrich-jsonl`. CLI [`tools.exp007_rpc_enrich`](tools/exp007_rpc_enrich.py); runbook [`tools/exp007b_oracle_run.md`](tools/exp007b_oracle_run.md). Host `/var/lib/mal/paper/_exp007b-oracle-*` (gitignored). **Merge ≠ continuous observe-wiring.** Stamp row in [EXP-007b](EXP/EXP-007b-platform-regime-rpc-enrich-v0.md) from host summaries.
