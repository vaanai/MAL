...
9. Hot-packet JSON spec (capped graph + market spine) aligned to matrix backfill rules
10. **Hard defer:** Birdeye paid; **Dexscreener debug enrich only**; bonk/mayhem reclass **parked**
11. Defer X API until social hypothesis has a cheap proxy or manual sample set (**no X keys on `mal-core-0`**)
12. Managers reload this file + latest [ARTIFACTS/SUMMARY.md](ARTIFACTS/SUMMARY.md) each session
