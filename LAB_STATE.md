...
6. **EXP-004 Graph Discovery:** Oracle full-book run **2026-09-23** on `mal-core-vnic` @ `9ca8ddb` — **INCOMPLETE** (honest): sealed `observe-2026-09-23.jsonl` **8553** lines / **~14.3 MiB**; **no EXP-003 marks on host**; population_n=**7475**; priced_60s_n=**2** (floor **10**); CLI exit **2**; on-host artifacts `/var/lib/mal/paper/_exp004-oracle_*` (gitignored). Primary **60s** gates: H-G1–G3 **INCOMPLETE**; H-G4 **N/A** (empty positive arm). **Next:** courier/laptop marks → re-run with `--marks` (no densify-for-power). CLI [`tools.exp004_graph_discovery`](tools/exp004_graph_discovery.py); [EXP-004](EXP/EXP-004-graph-creator-recurrence-v0.md); brief [GRAPH-DISCOVERY-V0.md](ARTIFACTS/GRAPH-DISCOVERY-V0.md).
